...
- [Document Table features](#document-table-features)
...
- `documents`: the document CSV file generated by the Topic Mapping Pipeline's export module.
...
These features let you access and interact with the document lists inside the `mainModel` and `subModel` to set up a [document table](#document-table).
...
These features let you access and interact with the document list in `documents`.

#### `DataManager.documents.filterRemovedDocs()`

Separates documents which have been excluded from the model into a separate list. The function will throw an error if the documents CSV file was not loaded.
```javascript
DM.documents.filterRemovedDocs();
console.log(DM.data.documents); // -> only the documents with '_inModel' == true
console.log(DM.data.removedDocuments); // -> documents with '_inModel' == false
```

#### `DataManager.documents.processDocumentTopics([percentage [, threshold]])`

For each documents, processes its main and sub topics information to: a) collapse it into two `_mainTopics` and `_subTopics` attributes (instead of one attribute per topic), and b) generate two string `topMainTopics` and `topSubTopics` the top main and sub topics for this document respectively. These string are in the format: `label1-label2-label3 (x%)`. `percentage` lets you pick with top percentile of topics to keep in the top topics strings, it defaults to `0.2`. `threshold` lets you specify what is the minimum weight a topic must have to be in the top topics strings, it defaults to `0.1`. The function also replaces the attribute name `_docId` to `docId`. The function will throw an error if the documents CSV file was not loaded.
```javascript
DM.documents.processDocumentsTopics(0.3, 0.2); // _. each top topic string get the top 30% of topics for each document, and they must have their weights above 0.2
console.log(DM.data.documents[0].topMainTopics); // -> 'label1-label2-label3 (60%) label4-label5-label6 (30%)
```

#### `DataManager.documents.shortenTexts(fields [, characterLimit])`

Lets you create short version of documents fields. `fields` is map indicating which field to shorten and which name the short field should have, e.g., `[['text','shortText']]`. `characterLimit` lets you define with length the short texts must have, in number of characters, it defaults to `200`. The function will throw an error if the documents CSV file was not loaded.
```javascript
DM.documents.shortenTexts([['text','shortText']], 300);
console.log(DM.data.documents[0].text); // -> full text
console.log(DM.data.documents[0].shortText); // -> text cut to 300 characters + '...'
```

#### `DataManager.documents.setSearchFields(fields)`

Lets you specify which document fields to index during searches. `fields` is the list of fields, e.g., ['field1', 'field2'].
```javascript
DM.documents.setSearchFields(['field1','field3']); // -> field1 and field3 will be the only fields to be indexed during search
```

#### `DataManager.documents.setSearchTerm([searchTerm])`

Lets you set a search term to filter documents. `searchTerm` is the string to search for in documents, it is optional, and defaults to `""`, i.e., no filter on documents.
```javascript
DM.documents.setSearchTerm('label'); // -> will filter documents with 'label' in their search fields
```

#### `DataManager.documents.getDocumentsToRender()`

Returns the documents which hae filtered by the search. The function will throw an error if the documents CSV file was not loaded.
```javascript
DM.documents.setSearchFields(['field2']);
DM.documents.setSearchTerm('label');
console.log(DM.documents.getDocumentsToRender()); // -> documents with 'label' in their field 'field2'
```
